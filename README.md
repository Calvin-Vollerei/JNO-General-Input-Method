# JNO-General-Input-Method
Convert text in real-time into a JNO-compatible input format. Supports Chinese and English, horizontal and vertical layouts, local font libraries, and adaptive scaling.
...
v1.7: Further fixed UI display issues when changing themes, refactored the underlying system, improved software startup speed, rewrote the output algorithm, optimized adaptive output effects, and reduced label performance overhead.

v1.8: Completely redesigned output byte options and compression algorithms to optimize performance costs. Further refactored UI infrastructure significantly improves cold app launch speed, with actual tests showing a 25% reduction in cold start time compared to v1.7. Fully rebuilt rendering algorithms reduce output load and provide support for 25-color swatches. Added color swatch functionality, supporting manual input of color codes, previewing color codes, locating JNO color swatches, and eliminating the need to recolor labels after output. Updated the help and about pages to provide users with clearer offline support.
