...
class FontManagerDialog(tk.Toplevel):
    ...
    def _bg(self): return (DARK_THEME if self.app.dark_theme else LIGHT_THEME)["list_bg"]
    def _fg(self): return (DARK_THEME if self.app.dark_theme else LIGHT_THEME)["list_fg"]
    ...
    def _build(self):
        ...
        def _fi(e):
            if self._filter_var.get() == ph:
                self._filter_var.set(""); e.widget.config(foreground=self._fg())
        def _fo(e):
            if not self._filter_var.get().strip():
                self._filter_var.set(ph); e.widget.config(foreground="gray")
        fe.bind("<FocusIn>", _fi); fe.bind("<FocusOut>", _fo)
        ...
        sb = ttk.Scrollbar(lc, orient=tk.VERTICAL, command=self._canvas.yview)
        self._sf = ttk.Frame(self._canvas)
        self._sf.bind("<Configure>", lambda e: self._canvas.configure(scrollregion=self._canvas.bbox("all")))
        cw = self._canvas.create_window((0, 0), window=self._sf, anchor=tk.NW)
        ...
        bf = ttk.Frame(self); bf.pack(fill=tk.X, padx=8, pady=5)
        ttk.Button(bf, text="应用", command=self._apply).pack(side=tk.LEFT, padx=2)
        ttk.Button(bf, text="关闭", command=self._close).pack(side=tk.LEFT, padx=2)

    def _populate(self):
        for w in self._sf.winfo_children(): w.destroy()
        self._check_vars.clear()
        q = self._filter_var.get().strip()
        if q == self.t("font_filter"): q = ""
        fq = [x for x in self._all_fonts if q.lower() in x[0].lower()] if q else self._all_fonts
        bg, fg = self._bg(), self._fg()
        for name, _ in fq:
            v = tk.BooleanVar(value=name not in self._original_disabled)
            self._check_vars[name] = v
            tk.Checkbutton(self._sf, text=name, variable=v, bg=bg, fg=fg,
                           selectcolor=bg, activebackground=bg, activeforeground=fg,
                           anchor=tk.W, padx=2).pack(anchor=tk.W, fill=tk.X, padx=5, pady=1)
        self._upd_cnt()
    ...
    def _upd_cnt(self):
        t = len(self._all_fonts); e = t - len(self._original_disabled)
        s = self.t("font_count", e, t)
        if self._has_changes(): s += "  *"
    ...
    def _apply(self):
        self.app.disabled_fonts = {n for n, v in self._check_vars.items() if not v.get()}
        self._original_disabled = set(self.app.disabled_fonts)
        self.app.save_settings()
        self.app._refresh_font_combo()
        self._upd_cnt()
        messagebox.showinfo(self.t("font_manager_title"), self.t("font_applied"), parent=self)

    def _close(self):
        if self._has_changes():
            if not messagebox.askyesno(self.t("font_manager_title"), self.t("font_unsaved"), parent=self):
                return
        self._cleanup(); self.destroy()
    ...
    def _cleanup(self):
        try:
            if hasattr(self, '_mwb'): self.unbind("<MouseWheel>", self._mwb)
